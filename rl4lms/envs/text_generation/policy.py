...
class LMActorCriticPolicy(BasePolicy, ActorCriticWarmStartMixin):
    ...
    def _setup_optimizer(self, optimizer_kwargs: Dict[str, Any], weight_decay: float, optimizer_class: torch.optim):
    ...
    def _prepare_inputs_for_model(
        ...
        if self._apply_model_parallel:
            ...
            model_inputs = {
                key: value.to(model.transformer.first_device) if isinstance(value, torch.Tensor) else value
    ...
    def get_distribution(self, obs: TensorDict, detach=False):
        ...
        if detach:
            with torch.no_grad():
                model_inputs = self._prepare_inputs_for_model(self._policy_model, input_ids, past_model_kwargs)
    ...
    def forward_policy(
        self, obs: TensorDict, actions: torch.tensor, past_model_kwargs: Optional[Dict[str, torch.tensor]] = None
    ):
        input_ids = obs["input_encoded_pt"].int()
        attention_mask = obs["input_attention_mask_pt"]
    ...
    def forward_value(self, obs: TensorDict, past_model_kwargs: Optional[Dict[str, torch.tensor]] = None):
        ...
        model_inputs = self._prepare_inputs_for_model(self._value_model, input_ids, past_model_kwargs)
    ...
    def evaluate_actions(
        ...
        _, log_prob, entropy, _, _ = self.forward_policy(obs=obs, actions=actions)
        values, _ = self.forward_value(obs)

        return values, log_prob, entropy

    def get_log_probs_ref_model(self, obs: TensorDict, action: torch.tensor, past_model_kwargs: Dict[str, Any] = None):
        ...
        model_inputs = self._prepare_inputs_for_model(self._ref_model, input_ids, past_model_kwargs)
...
class Seq2SeqLMActorCriticPolicy(LMActorCriticPolicy):
    ...
    def forward_policy(
        self, obs: TensorDict, actions: torch.tensor, model_kwargs: Optional[Dict[str, torch.tensor]] = None
    ):
        ...
        model_inputs = unwrap_model(self._policy_model).prepare_inputs_for_generation(input_ids, **model_kwargs)
    ...
    def forward_value(self, obs: TensorDict, model_kwargs: Optional[Dict[str, torch.tensor]] = None):
        if model_kwargs is None:
            # 1. prepare model inputs
            model_kwargs = {
                "attention_mask": obs["prompt_or_input_attention_mask_pt"],
        ...
        # prepare all of the model inputs for the decoder
        batch_size = input_ids.shape[0]
        model_inputs = unwrap_model(self._value_model).prepare_inputs_for_generation(input_ids, **model_kwargs)
    ...
    def get_log_probs_ref_model(self, obs: TensorDict, action: torch.tensor, model_kwargs: Dict[str, Any] = None):
        if model_kwargs is None:
            # 1. prepare model inputs
            model_kwargs = {
                "attention_mask": obs["prompt_or_input_attention_mask_pt"],
        ...
        # prepare all of the model inputs for the decoder
        batch_size = input_ids.shape[0]
        model_inputs = unwrap_model(self._ref_model).prepare_inputs_for_generation(input_ids, **model_kwargs)
...
class MaskableLMActorCriticPolicy(BasePolicy, MaskableActorCriticWarmStartMixin):
    ...
    def _get_action_masks(self, input_ids: torch.tensor, attention_mask: torch.tensor) -> torch.tensor:
        action_masks = torch.zeros((input_ids.size(0), self.action_space.n)).to(self.device)
        model_kwargs = {
            "attention_mask": attention_mask,
        }
        model_inputs = self._prepare_inputs_for_model(self._mask_model, input_ids, model_kwargs)
        output = self._mask_model(output_hidden_states=True, **model_inputs)

        next_token_logits = output.logits[:, -1, :]
        ref_distr = self._mask_action_dist.proba_distribution(action_logits=next_token_logits)
        next_token_probs = ref_distr.distribution.probs
        _, topk_indices = torch.topk(next_token_probs, k=self.top_mask, dim=1, sorted=True)
    ...
    def evaluate_actions(
        ...
        _, log_prob, entropy, _, _, _ = self.forward_policy(obs=obs, actions=actions, action_masks=action_masks)
        values, _ = self.forward_value(obs)
    ...
    def get_log_probs_ref_model(self, obs: TensorDict, action: torch.tensor, past_model_kwargs: Dict[str, Any] = None):
        ...
        output = self._ref_model(output_hidden_states=True, **model_inputs)
        next_token_logits = output.logits[:, -1, :]
        dist = self._ref_action_dist.proba_distribution(action_logits=next_token_logits)
        log_prob = dist.log_prob(action)
    ...
    def generate(
        ...
        self.logits_processor.attention_mask = attention_mask.to(self.get_policy_first_device())
        self.logits_processor.all_special_ids = self.all_special_ids = (
            torch.tensor(tokenizer.all_special_ids, dtype=input_ids.dtype, device=self.get_policy_first_device())
            .unsqueeze(0)
...
class MaskableSeq2SeqLMActorCriticPolicy(MaskableLMActorCriticPolicy):
    ...
    def _get_action_masks(self, model_inputs, decoder_attn_mask) -> torch.tensor:
        action_masks = torch.zeros((decoder_attn_mask.size(0), self.action_space.n)).to(self.device)
        outputs = self._mask_model(**model_inputs, decoder_attention_mask=decoder_attn_mask, return_dict=True)
        next_token_logits = outputs.logits[:, -1, :]
        ref_distr = self._action_dist.proba_distribution(action_logits=next_token_logits)
        next_token_probs = ref_distr.distribution.probs
        _, topk_indices = torch.topk(next_token_probs, k=self.top_mask, dim=1, sorted=True)
        action_masks = action_masks.scatter(index=topk_indices.long(), dim=1, value=1)
