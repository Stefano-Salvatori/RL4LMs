...
def build_env(
    env_config: Dict[str, Any], reward_fn: RewardFunction, tokenizer: AutoTokenizer, train_samples: List[Sample]
):
    # vectoried env
    env_kwargs = {
    ...
    env = make_vec_env(
        TextGenEnv, n_envs=env_config.get("n_envs", 1), vec_env_cls=SubprocVecEnv, env_kwargs=env_kwargs
    )
    return env
...
class OnPolicyTrainer(TrainerWarmStartMixin):
    ...
    def _setup(self):
        ...
        self._env = build_env(self._env_config, self._reward_fn, self._tokenizer, self._samples_by_split["train"])
        self._alg = build_alg(
            self._on_policy_alg_config, self._env, self._tracker, self._policy_state_dict, self._alg_state_dict
        )
...
class SupervisedTrainer:
    ...
    def _setup(self):
        ...
        if (self._alg_config["model_type"] == "causal") and (
            ...
            self._max_prompt_length = self._max_prompt_length - self._gen_kwargs["max_new_tokens"]
